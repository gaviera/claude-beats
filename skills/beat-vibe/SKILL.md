...
**A mood description:** If the user describes a vibe instead of naming a preset (e.g., "something jazzy and dark"), compose a custom beat definition using the JSON schema from the beat-generation-guide rule and write it to mood.json.

Steps:
1. Parse the argument: $ARGUMENTS
2. If it matches a preset name, write `{"preset": "name"}` to mood.json
3. If it's a description, compose a custom beat JSON following the rules in `.claude/rules/beat-generation-guide.md` and write it to mood.json
4. The engine will detect the change and crossfade automatically
